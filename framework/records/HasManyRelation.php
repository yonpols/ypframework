<?php
    class HasManyRelation extends ModelBaseRelation implements IModelQuery, Iterator {
            ...
            foreach($this->relationParams['keys'] as $index=>$key)
            {
                if (is_numeric($index))
                    $whereConditions[] = sprintf('(%s.%s = %s)',
                        ...
                        ModelBase::getFieldSQLRepresentation($this->relatorModelParams->keyFields[$index], $relatorModel->__get($this->relatorModelParams->keyFields[$index]), $this->relatorModelParams));
                else
                    $whereConditions[] = sprintf('(%s.%s = %s)',
                        $this->relationName, $key,
                        ModelBase::getFieldSQLRepresentation($index, $relatorModel->{$index}, $this->relatorModelParams));
            }
